...
Here are the initial and final states of a few more small programs:
    ...
    1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2).  2,3,0,3,99 becomes 2,3,0,6,99
(3 * 2 = 6).  2,4,4,5,99,0 becomes 2,4,4,5,99,9801 (99 * 99 = 9801).
...
use std::cmp::min;
pub fn aoc2b_run(needle: u32, tape: &[u32]) -> Option<(u32, u32)> {
    for noun in 0..min(100, tape.len() as u32) {
        for verb in 0..min(100, tape.len() as u32) {
            let mut this_tape: Vec<u32> = tape.to_vec();
            this_tape[1] = noun;
            this_tape[2] = verb;
            let res = aoc2a_run(this_tape);
            if res[0] == needle {
...
mod tests {
    ...
    #[test]
    fn aoc2a_eg() {
        assert_eq!(aoc2a_run(vec!(1, 0, 0, 0, 99)), vec!(2, 0, 0, 0, 99));
        assert_eq!(aoc2a_run(vec!(2, 3, 0, 3, 99)), vec!(2, 3, 0, 6, 99));
        assert_eq!(
            aoc2a_run(vec!(2, 4, 4, 5, 99, 00)),
            vec!(2, 4, 4, 5, 99, 9801)
        );
        assert_eq!(
            aoc2a_run(vec!(1, 1, 1, 4, 99, 5, 6, 0, 99)),
            vec!(30, 1, 1, 4, 2, 5, 6, 0, 99)
        );
        assert_eq!(
            aoc2a_run(vec!(1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50)),
            vec!(3500, 9, 10, 70, 2, 3, 11, 0, 99, 30, 40, 50)
        );
    }
    #[test]
    fn aoc2a_prob() {
        let in_nums: Result<Vec<u32>, std::num::ParseIntError> = include_str!("test_input.txt")
            .lines()
            .collect::<String>()
            .split(",")
            .map(|s| s.parse::<u32>())
        ...
        in_nums[1] = 12;
        in_nums[2] = 2;
        let res = aoc2a_run(in_nums);
        assert_eq!(res[0], 5290681);
    }
    #[test]
    fn aoc2b_eg() {
        assert_eq!(aoc2b_run(99, &vec!(2, 0, 0, 0, 99)), Some((1, 4)));
    }
    #[test]
    fn aoc2b_prob() {
        let in_nums: Result<Vec<u32>, std::num::ParseIntError> = include_str!("test_input.txt")
